# -*- coding: utf-8 -*-
"""
Created on Tue May 26 20:19:40 2026

@author: Afshin-III + ChatGPT 5
"""
"""
Elephant V4.0 - GUI Design Matched Ethernet Foundation
Author: Afshin + ChatGPT

Purpose:
    Upgrade Elephant V3.x style GUI from USB to Ethernet while keeping the
    visual structure of the Elephant V4.0 GUI design file.

System:
    PC / Control Room
    │
    └── LAN Ethernet Switch
        ├── Node0 / Arduino Mega 2560 + W5100
        └── Power Supply #1 / IT-M3233

GUI layout, matched to Elephant V4.0 sketch:
    LEFT  : Control and monitoring
            - Coil #1
            - Helmholtz Coil
            - Power Supply #1

    RIGHT : Connection and tools
            - Node0 / Coil #1 IP section
            - Power Supply #1 IP section
            - Terminal to Node0
            - Log box

Node0 hardware map reference:
    Coil #1 measurements:
    ...
    Helmholtz Coil measurements:
        ADC14 -> hc_crt_msr / hc_dc_crt_msr
        ADC15 -> hc_temp_msr
        D30   -> hc_pw_msr
        D31   -> hc_pol_msr
    ...
    Coil #1 controls:
        D36   -> coil_1_pw_stat
        D37   -> coil_1_pol_stat
        MCP41010 #1 CS D3 -> coil_1_dc_crt_stat
        MCP41010 #2 CS D5 -> coil_1_ac_crt_stat
        AD9833 FSYNC D2   -> coil_mod_freq_stat, coil_wf_stat

    Helmholtz Coil controls:
        D44   -> hc_pw_stat
        D45   -> hc_pol_stat
        MCP41010 #9 CS D14 -> hc_crt_stat

First protocol assumptions, to be finalized after latest Arduino firmware upload:
    GUI -> Node0:
        PING
        GET_STATUS
        SET_COIL coil_pw_set=1 coil_pol_set=0 coil_dc_crt_set=2.5 coil_ac_crt_set=100 mod_freq_set=1000 wf_set=Sin
        SET_HC hc_pw_set=1 hc_pol_set=0 hc_dc_crt_set=1.5

    Node0 -> GUI:
        PONG
        OK
        ERR message
        STATUS coil_pw_msr=1 coil_pol_msr=0 coil_dc_crt_msr=2.48 coil_ac_crt_msr=97 coil_temp_msr=31 hc_pw_msr=1 hc_pol_msr=0 hc_dc_crt_msr=1.49 hc_temp_msr=29
...
NODE0_SOCKET_TIMEOUT_S = 1.0
POLL_INTERVAL_MS = 500

# Current Arduino diagnostic firmware accepts STATUS, not GET_STATUS yet.
NODE0_STATUS_COMMAND = "STATUS"
...
class SystemState:
    ...
    coil_ac_crt_set: int = 0       # mA
    mod_freq_set: int = DEFAULT_MOD_FREQ_HZ
    wf_set: str = "Sin"

    # Coil #1 measured values from Node0
    coil_pw_msr: int = 0
    coil_pol_msr: int = 0
    coil_dc_crt_msr: float = 0.0
    coil_ac_crt_msr: int = 0       # mA
    ...
    # HC measured values from Node0
    hc_pw_msr: int = 0
    hc_pol_msr: int = 0
    hc_dc_crt_msr: float = 0.0
    hc_temp_msr: float = 0.0

    # Power supply setpoints/status
    ps_pw_set: int = 0
...
class ElephantV40(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Elephant V4.0 - Ethernet Control GUI")
    ...
    def _build_ui(self):
        ...
        left = self._build_left_control_panel()
        right = self._build_right_connection_panel()
    ...
    def _build_left_control_panel(self):
        ...
        title = QLabel("Elephant V4.0")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        layout.addWidget(self._build_coil1_box())
        layout.addWidget(self._build_hc_box())
        layout.addWidget(self._build_ps_box())
        layout.addStretch()
        return panel

    def _build_right_connection_panel(self):
        panel = QWidget()
        layout = QVBoxLayout(panel)
    ...
    def _build_coil1_box(self):
        ...
        self.coil_temp_set_blank = ReadOnlyLine("-")
        ...
        self.mod_freq_msr_blank = ReadOnlyLine("-")
        self.wf_msr_blank = ReadOnlyLine("-")
        ...
        rows = [
            ...
            ("Modulation Frequency (Hz)", self.mod_freq_set, self.mod_freq_msr_blank),
            ("Waveform", self.wf_set, self.wf_msr_blank),
            ("Temperature (°C)", self.coil_temp_set_blank, self.coil_temp_msr),
            ("Power Loss (W)", self.coil_loss_set_cal, self.coil_loss_msr_cal),
        ]

        for r, (name, set_w, msr_w) in enumerate(rows, start=1):
            grid.addWidget(QLabel(name), r, 0)
            grid.addWidget(set_w, r, 1)
            grid.addWidget(msr_w, r, 2)
        ...
        grid.addWidget(self.btn_clear_coil, len(rows) + 1, 1)
        grid.addWidget(self.btn_update_coil, len(rows) + 1, 2)

        return box

    def _build_hc_box(self):
        ...
        self.hc_temp_set_blank = ReadOnlyLine("-")
        ...
        rows = [
            ...
            ("Temperature (°C)", self.hc_temp_set_blank, self.hc_temp_msr),
        ]
        ...
        grid.addWidget(self.btn_clear_hc, len(rows) + 1, 1)
        grid.addWidget(self.btn_update_hc, len(rows) + 1, 2)

        return box

    def _build_ps_box(self):
        ...
        grid.addWidget(self.btn_clear_ps, len(rows) + 1, 1)
        grid.addWidget(self.btn_update_ps, len(rows) + 1, 2)
    ...
    def _build_connection_box(self):
        ...
        grid.addWidget(node_title, 0, 0, 1, 4)
        ...
        grid.addWidget(self.btn_connect_ps, 6, 3)
        grid.addWidget(QLabel("Port"), 7, 0)
        grid.addWidget(self.port_ps, 7, 1)

        self.btn_disconnect_all = QPushButton("Disconnect All")
        grid.addWidget(self.btn_disconnect_all, 8, 2, 1, 2)

        return box

    def _build_terminal_box(self):
        box = QGroupBox("Terminal")
        layout = QHBoxLayout(box)
        self.terminal_input = QLineEdit()
        self.terminal_input.setPlaceholderText("Send command to Node0, e.g. GET_STATUS")
    ...
    # Ping
    # ========================================================
    def run_ping(self, ip: str, label: str):
        ip = ip.strip()
        if not ip:
            self.add_log(f"{label} ping failed: empty IP address")
            return False
        ...
        try:
            ...
            if result.returncode == 0:
                self.add_log(f"PING {label}: SUCCESS")
                return True
            self.add_log(f"PING {label}: FAILED")
            return False
        except subprocess.TimeoutExpired:
            self.add_log(f"PING {label}: TIMEOUT")
            return False
    ...
    def ping_node0(self):
        ...
        self.node0_lamp.set_state("on" if ok else "fault")
        self.node0_lamp.setText("Node0 OK" if ok else "Node0 FAULT")

    def ping_ps(self):
        self.read_gui_to_state()
        ok = self.run_ping(self.state.ip_ps, "IT-M3233")
        self.ps_lamp.set_state("on" if ok else "fault")
        self.ps_lamp.setText("PS OK" if ok else "PS FAULT")
    ...
    def node0_transaction(self, command: str, expect_response=True):
    ...
    def connect_node0(self):
        if self.node0_open_socket():
            ok, response = self.node0_transaction("PING", expect_response=True)
            if ok and response.strip().upper().startswith("PONG"):
                self.add_log("Node0 connection test SUCCESS.")
            else:
                self.add_log("Node0 connected, but PING/PONG test did not match expected response.")
    ...
    def send_terminal_to_node0(self):
        ...
        if ok and response.startswith("STATUS"):
            if "=" in response:
                self.parse_node0_status(response)
            else:
                self.add_log("Node0 diagnostic STATUS received.")
    ...
    def ps_open_socket(self):
        ...
        try:
            ...
            self.add_log(f"PS socket open failed: {exc}")
            return False

    def ps_transaction(self, command: str, expect_response: bool = True):
    ...
    def update_coil(self):
        ...
        self.node0_transaction(command, expect_response=True)
    ...
    def update_ps(self):
        ...
        self.add_log("Update PS: commands sent successfully." if all_ok else "Update PS: one or more commands failed.")
    ...
    def poll_status(self):
        if self.state.node0_connected and self.node0_sock is not None:
            ok, response = self.node0_transaction(NODE0_STATUS_COMMAND, expect_response=True)
            if ok and response.startswith("STATUS"):
                # Diagnostic firmware currently returns:
                # STATUS Ethernet diagnostic firmware running
                # Later firmware will return key=value measurement fields.
                if "=" in response:
                    self.parse_node0_status(response)
                else:
                    self.add_log("Node0 diagnostic STATUS received.")
        if self.state.ps_connected and self.ps_sock is not None:
            # Keep PS polling manual for now to avoid overloading SCPI communication.
            pass

    # ========================================================
    # Parsing
    # ========================================================
    def parse_node0_status(self, line: str):
        values = self._parse_key_value_line(line)

        # Accept both GUI design variable names and hardware-map variable names.
        alias = {
    ...
    # ========================================================
    # Calculations and clear buttons
    # ========================================================
    ...
    def calc_channel_loss(ps_voltage: float, ps_current: float) -> float:
        loss_voltage = ps_voltage - ps_current * RES_COIL_1_OHM
        return max(ps_current * loss_voltage, 0.0)
    ...
    def save_log_placeholder(self):
        self.add_log("Save log placeholder. File saving can be added after communication is stable.")
